...
resource "aws_ecs_service" "ECSServiceWithoutLB" {
  count                              = false == var.lb_enabled ? 1 : 0
  name                               = var.service_name
  cluster                            = var.cluster_id
  task_definition                    = var.task_definition_arn
  desired_count                      = var.desire_count
...
resource "aws_appautoscaling_target" "AppAsgTarget" {
  ...
  resource_id = var.lb_enabled ? format(
    "%s%s%s%s",
    "service/",
    var.cluster_id,
    "/",
    join(" ", aws_ecs_service.ECSService.*.name),
    ) : format(
    "%s%s%s%s",
    "service/",
    var.cluster_id,
    "/",
    join(" ", aws_ecs_service.ECSServiceWithoutLB.*.name),
  )
...
resource "aws_cloudwatch_metric_alarm" "EcsLowAlarm" {
  ...
  alarm_actions     = [aws_appautoscaling_policy.scale-down[0].arn]
}
...
resource "aws_cloudwatch_metric_alarm" "EcsHighAlarm" {
  ...
  alarm_description = "This metric monitors ecs cpu utilization"
  alarm_actions     = [aws_appautoscaling_policy.scale-up[0].arn]
}
